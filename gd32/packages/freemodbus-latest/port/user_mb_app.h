...
/* user config */
#define NUM_REG_DISCRETES      (RT_ALIGN(S_DISCRETE_INPUT_NDISCRETES, 8)/8) 
#define NUM_REG_COILS          (RT_ALIGN(S_COIL_NCOILS, 8)/8) 
#define MB_BUF_LEN             (NUM_REG_DISCRETES + NUM_REG_COILS + (S_REG_INPUT_NREGS)*2 + (S_REG_HOLDING_NREGS)*2)

extern UCHAR modbus_reg_bufs[MB_BUF_LEN];
#endif
